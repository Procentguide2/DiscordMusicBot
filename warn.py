...
class Warn(commands.Cog):
    ...
    async def warn_command(self, ctx, user: discord.Member, *, reason: str):
        ...
        if user.bot == 1:
            ...
            await ctx.send("It's useless to warn a bot.")
        ...
        if user == ctx.author:
            ...
            await ctx.send("Can't give a warning to yourself")
        ...
        f = open(f"data/warns/{str(ctx.guild.id)}/{str(user.id)}.json", "w")

        warn_amount = data.get("warns")
        new_warn_amount = warn_amount + 1

        if new_warn_amount == 3:
            await user.ban(reason=reason)
            await ctx.send("You have been banned for excessively violating chat rules")
        ...
        json.dump(data, open(f"data/warns/{str(ctx.guild.id)}/{str(user.id)}.json", "w"))
    ...
    async def remove_warn_command(self, ctx, user: discord.Member, *, warn: str):
        ...
        if reply in ('y', 'yes', 'confirm'):
            ...
            if warn_amount == 1:  # Check if the user only has one warn.
                os.remove("data/warns/" + str(ctx.guild.id) + "/" + str(
    ...
    async def edit_warn_command(self, ctx, user: discord.Member, *, warn: str):
        ...
        msg = await self.bot.wait_for('message',
                                      check=check)  # Pause the ongoing script to wait for the user to send a message.
